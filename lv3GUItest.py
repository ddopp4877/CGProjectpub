...
class Window:
    def __init__(self,master):
        ...
        self.usedParams = np.delete(myCell.LV2CellControl.params,10,axis=0).reshape(-1,1)
        
        ar1 = np.array([0.0008000,.400000,.160000000]).reshape(-1,1)
...
class CellPlot:
    ...
    #start by running the control and tea sims and setting plotting constants
        ...
        self.TEAPlot = self.plotClass("TEA",self.simTime,self.plotArrayTEA)
                #create Slider objects
        self.synGainSliderTEA = self.mySliders(0.16,2,0.06,0.09,"synGain")#start, end, step, yaxis location, labelname
    ...
    class plotClass:
        def __init__(self,controlorTEA,simTime,plotArrayl):#pass it the type of run and the data to plot
            ...
            if(controlorTEA == "Control"):
                self.canvas.get_tk_widget().grid(column = plotCols, row=0,rowspan=19,padx = 330,pady=15)
            elif(controlorTEA== "TEA"):
                self.canvas.get_tk_widget().grid(column = plotCols, row=19,rowspan=19,padx = 330,pady=15)

cellNo,FreqNo = 3,16 #network number ?, tested at ? Hz
